...
import { getAnnoucement, sendTrade } from '@/service/api/products'
  ...
  methods: {
    ...
    handleCheckAllChange (val) {
      ...
      sendMessage () {
        ...
        //
        let obj = {'email': this.newEmail, 'checkedList': this.checkedList, 'userId': this.newUserId, 'type': 2}
  ...
  mounted () {
    ...
    this.newUserId = JSON.parse(window.localStorage.getItem('data')).userId
      ...
      this.documentList.map((item, index) => {
        ...
        nameList.push({label: item.name, value: item.announcement_file_id})
        urlList.push(item.announcement_file_id)
      })
      this.newList = list
